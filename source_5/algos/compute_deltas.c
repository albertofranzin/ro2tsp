...
int compute_deltas(graph* G, onetree* OT) {
  ...
  for (i = 1; i < n; i++) {
    if (i != OT->nn1 && i != OT->nn2 && OT->V[i].deg == 1) { // 1st and 2nd condition are redundant
      leaves[numleaves++] = i;
    }
  }
  ...
  for (i = 1; i < n; i++) {
	for (j = i+1; j < n; j++) {
		...
		if (p == u) { // the edge belongs to the 1-tree
			...
			if (delta_pu > cost_pv) graph_set_edge_delta(G, u, v, delta_pu);
			else			graph_set_edge_delta(G, u, v, cost_pv);
