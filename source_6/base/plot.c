...
int plot_tree(tree* t, coords* crds, char* title) {
	...
	for (i = 0; i < crds->num; i++) {
		if (t->vrtx_pred[i] >= 0) {
			graph_insert_edge(&gtemp, t->vrtx_pred[i], i, 0.0, FREE);
		}
	}
...
int plot_cycle(cycle* c, coords* crds, char* title) {
...
#ifdef DEBUG
    if (c->vrtx_num != crds->num) {
        fprintf(stderr, "Error in /base/plot.c:\n"
                        "function: plot_cycle:\n"
                        "plot_cycle : errcode %d\n", 1);
        return -1;
    }
#endif
	...
	for (i = 0; i < crds->num; i++) {
		graph_insert_edge(&gtemp, c->vrtx_idx[i], c->vrtx_idx[(i+1)%(crds->num)], 0.0, FREE);
	}
